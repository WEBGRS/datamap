...
const SAMPLES = {
  "us-gdp": {
    label: "美国 · 各州 GDP 2025", map: "us-states",
    title: "美国各州 GDP · 2025", subtitle: "名义 GDP，十亿美元 (Nominal GDP, $ billions)",
    unit: "十亿美元", fmtStyle: "compact", method: "jenks", kind: "sequential", hue: "blue",
    source: "U.S. Bureau of Economic Analysis (BEA), 2025",
    csv: csv("州 State,GDP（十亿美元）", us.map((r) => `${q(r.name)},${round(r.gdp2025 / 1000, 1)}`)),
  },
  "us-gdp-pc": {
    label: "美国 · 各州人均 GDP 2025", map: "us-states",
    title: "美国各州人均 GDP · 2025", subtitle: "名义人均 GDP，美元 (Nominal GDP per capita, US$)",
    unit: "", fmtStyle: "currency", method: "quantile", kind: "sequential", hue: "teal",
    source: "BEA + U.S. Census Bureau, 2025",
    csv: csv("州 State,人均 GDP（美元）", us.map((r) => `${q(r.name)},${Math.round(r.pcap2025)}`)),
  },
  "us-growth": {
    label: "美国 · 各州实际 GDP 增速", map: "us-states",
    title: "美国各州实际 GDP 增速 · 2024→2025",
    ...
    csv: csv("州 State,增速（%）", us.map((r) => `${q(r.name)},${round(r.growth, 1)}`)),
  },
  "us-pop": {
    label: "美国 · 各州人口 2025", map: "us-states",
    title: "美国各州人口 · 2025", subtitle: "常住人口估计 (Resident population estimate)",
    unit: "人", fmtStyle: "compact", method: "jenks", kind: "sequential", hue: "violet",
    source: "U.S. Census Bureau, Population Estimates Program",
    csv: csv("州 State,人口", us.map((r) => `${q(r.name)},${Math.round(r.pop)}`)),
  },
  "county-pop": {
    label: "美国 · 各县人口 2024", map: "us-counties",
    title: "美国各县人口 · 2024", subtitle: "3,142 个县级行政区，对数分级 (county population, log scale)",
    unit: "人", fmtStyle: "compact", method: "log", kind: "sequential", hue: "blue",
    source: "U.S. Census Bureau, Vintage 2024 county estimates（康涅狄格 8 县因 2022 年改划规划区，底图无对应单元）",
    csv: csv("FIPS,人口", counties.map((r) => `${r.fips},${r.pop}`)),
  },
  "county-growth": {
    label: "美国 · 各县人口变化 2020→24", map: "us-counties",
    title: "美国各县人口变化 · 2020→2024", subtitle: "相对 2020 普查基数的变化 (% change vs 2020 base)",
    unit: "%", fmtStyle: "number", method: "diverging", kind: "diverging", hue: "blue",
    decimals: 1, center: 0,
    source: "U.S. Census Bureau, Vintage 2024 county estimates（康涅狄格 8 县因 2022 年改划规划区，底图无对应单元）",
    csv: csv("FIPS,变化（%）", counties.filter((r) => Number.isFinite(r.growth)).map((r) => `${r.fips},${round(r.growth, 2)}`)),
  },
  "world-gdp": {
    label: "世界 · 各国 GDP 2024", map: "world",
    title: "世界各国 GDP · 2024", subtitle: "名义 GDP，十亿美元，对数分级 (Nominal GDP, US$ bn, log scale)",
    unit: "十亿美元", fmtStyle: "compact", method: "log", kind: "sequential", hue: "blue",
    source: "World Bank, World Development Indicators (NY.GDP.MKTP.CD)",
    csv: csv("国家 Country,GDP（十亿美元）", wbGdp.map((r) => `${q(r.iso3)},${round(r.value / 1e9, 2)}`)),
  },
  "world-gdp-pc": {
    label: "世界 · 各国人均 GDP 2024", map: "world",
    title: "世界各国人均 GDP · 2024", subtitle: "名义人均 GDP，美元 (GDP per capita, current US$)",
    unit: "", fmtStyle: "currency", method: "quantile", kind: "sequential", hue: "teal",
    source: "World Bank, World Development Indicators (NY.GDP.PCAP.CD)",
    csv: csv("国家 Country,人均 GDP（美元）", wbPcap.map((r) => `${q(r.iso3)},${Math.round(r.value)}`)),
  },
  "world-pop": {
    label: "世界 · 各国人口 2024", map: "world",
    title: "世界各国人口 · 2024", subtitle: "总人口，对数分级 (Total population, log scale)",
    unit: "人", fmtStyle: "compact", method: "log", kind: "sequential", hue: "violet",
    source: "World Bank, World Development Indicators (SP.POP.TOTL)",
    csv: csv("国家 Country,人口", wbPop.map((r) => `${q(r.iso3)},${Math.round(r.value)}`)),
  },
  "world-life": {
    label: "世界 · 各国预期寿命 2023", map: "world",
    title: "世界各国预期寿命 · 2023", subtitle: "出生时预期寿命，岁 (Life expectancy at birth, years)",
    unit: "岁", fmtStyle: "number", decimals: 1, method: "quantile", kind: "sequential", hue: "orange",
    source: "World Bank, World Development Indicators (SP.DYN.LE00.IN)",
    csv: csv("国家 Country,预期寿命（岁）", wbLife.map((r) => `${q(r.iso3)},${round(r.value, 1)}`)),
  },
};
